...
describe("Test 4 Busines logic of LazyHorse2 Drop contract", function () {
  let lazyHorse;
  let mockMemberships;
  let accounts;
  const token1Count = 5;
  ...
  beforeEach(async function () {
    ...
    const LazyHorse = await ethers.getContractFactory("LazyHorse2");
  ...
  it("Should return tokenURI", async function () {
    ...
    const tokenURI = await lazyHorse.tokenURI(2001);
    expect(tokenURI).to.be.equal("https://www.lazyhorseraceclub.com/lhrcmetadata/2001.json");
  });
